import React, { useContext } from 'react'
import i18next from 'i18next';
import { Link } from 'react-router-dom';
import Navbar from '../../components/Navbar';
import img from '../../images/Group 427 1.png'
import Footer from '../../components/Footer'
import '../../styles/css/login/login.-RestPass-ConfirmCode.css'
import { AppContext } from '../../contexts/AppContext';

function SignInSendResetCode() {
  const {lang , direction} = useContext(AppContext)
  i18next.init({
    lng: 'en',
    // debug: true,
    resources: {
      en: {
        translation: {
          "h1" : "Reset Password",
          "p" : "Enter your email address that you register with to receive your reset password code, check your email box",
          "emailLabel" : "Registered Email",
          "emailLabelPlaceholder" : "Enter your Registration Email ?",
          "btn" : "Send Code",
          "trylog" : "Try login again ?",
          "loginLink": " Click here",
          "haventAcc" : "Don't have an account ?",
          "Join" : "Join Us",
        }
      }, 
      ar: {
        translation: {
          "h1" : "إعادة تعيين كلمة المرور",
          "p" : "أدخل عنوان بريدك الإلكتروني الذي قمت بالتسجيل به لتتلقى رمز إعادة تعيين كلمة المرور، تفقد صندوق البريد الإلكتروني الخاص بك",
          "emailLabel" : "البريد الإلكتروني المسجل",
          "emailLabelPlaceholder" : "قم بإدخال بريدك الإلكتروني ؟",
          "btn" : "إرسال الرمز",
          "trylog" : "حاول تسجيل الدخول مرة أخرى ؟",
          "loginLink": "اضغط هنا",
          "haventAcc" : "ليس لديك حساب ؟ ",
          "Join" : "إنضم الأن",
        }
      }
    }
  });
  const handleSubmit = (event , url) => {
    event.preventDefault()
    window.location.assign(url)
  }
  return (
    <React.Fragment>
      <div className="resetpass" style={{direction:direction}}>
        <Navbar
          showhomeNavbarBtn={false}
          showThemeLangBtns={false}
          showJoinUsBtn={true}
        />
        <div className="container">
          <div className="conten">
          <div className="text">
            <h1>{i18next.t("h1" , {lng : lang})}</h1>
            <p>{i18next.t("p" , {lng : lang})}</p>
              <form action="">
                <div className="input">
                  <label htmlFor="email">{i18next.t("emailLabel" , {lng : lang})}</label>
                  <input id='email' type="email" placeholder={i18next.t("emailLabelPlaceholder" , {lng : lang})} autoFocus required/>
                </div>
                  <button type='submit' onClick={(event , url)=>handleSubmit(event , '/confirmresetcode')} className="submitBtn special">{i18next.t("btn" , {lng : lang})}</button>
                <p>{i18next.t("trylog", { lng: lang })} <span><Link to='/login'>{i18next.t("loginLink", { lng: lang })}</Link></span></p>
              </form>
            </div>
            <div className="image">
              <img src={img} alt="students" loading='lazy'/>
            </div>
          </div>
        <div className="linkwithbtnrespon">
          <p>{i18next.t("haventAcc" , {lng : lang})}</p>
          <button  onClick={(event , url)=>handleSubmit(event , '/')}>{i18next.t("Join" , {lng : lang})}</button>
        </div>
        </div>
        <Footer lang={lang} />
      </div>
    </React.Fragment>
  )
}

export default SignInSendResetCode
